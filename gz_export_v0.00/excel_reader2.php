...
class Spreadsheet_Excel_Reader {
	...
	function read($sFileName) {
		...
		if($res === false) { //ob_clean(); var_dump(debug_backtrace()); die();
			// check error code
...
//						$bgcolor = ($bgi & 0x3f80) >> 7;
	...
	function _encodeUTF16($string) { //return $string;
		//$source_encode = 'UTF-16LE';
		//$source_encode = 'ISO-8859-1'; //Western European
		//$source_encode = 'UTF-16';
		$source_encode = 'UTF-8';
		//$source_encode = null;
		//$source_encode = mb_detect_encoding($string); //die($source_encode);
		$result = $string;
		if ($this->_defaultEncoding){
			switch ($this->_encoderFunction){
				case 'iconv' :	 $result = iconv($source_encode, $this->_defaultEncoding, $string);
								break;
				case 'mb_convert_encoding' :	 $result = mb_convert_encoding($string, $this->_defaultEncoding, $source_encode );
								break;
			}
		}
		return $result;
	...
	//20110623:Tony:Added from http://code.google.com/p/php-excel-reader/wiki/Documentation
	...
	function getSheetName($i) {
		return $this -> boundsheets[$i]['name'];
	} 
}
