...
class gz_stock extends gz_tpl{
		...
		$config = [
			...
			,'shortcodes' => [
		  	...
		  	'ajaxes' => [
		 		...
		 		['prm'=>['get_import_list',[$this,'get_import_list']]],
			]
		];
	...
	function get_dataset_items(){
		$doc_root = $_SERVER["DOCUMENT_ROOT"];
		$att_src_path = '_stock/price_raw/';
		$att_set_name = $_GET['set'];
		$att_select = '*';
		$path = $doc_root.$att_src_path.$att_set_name.'/'.$att_select; //die('<pre>'.print_r(compact('path','atts','att_year','att_src_path'),true));
		$files = glob($path); //die('<pre>'.print_r(compact('path','files','x'),true));
		return($files);
	}
	
	function gz_import_list(){
		$html = '';
		$files = $this->get_dataset_items();
		$html.= '<pre>'.print_r($files,true).'</pre>';
		return $html;
	}
	
	function get_import_list(){
		if(null!=($get=$_GET['get']) && ($get=='html')) {echo $this->gz_import_list(); die(0);}
		else{
			$rs = (object)[
				'html'	=> $this->gz_import_list(),
			];
			echo json_encode($rs); die(0);
		}
	}
	...
	function gz_db_import_tabs(){
		$html_tabs = '';
		$html_pans = '';
		$datasets = $this->get_list_dataset();
		$html.= "<div class='db_import_tabs gz_panel' data-type='tabs'>";
		$url = site_url('/wp-admin/admin-ajax.php?action=get_import_list&get=html&set=');
		foreach($datasets as $dataset){
			$label = basename($dataset);
			$html_tabs.= "<li rel='{$label}'><a href='{$url}{$label}'>{$label}</a></li>";
	...
	function gz_stock_panel($atts,$content=false){
		$html = "";
		$html.= "<div class='gz_stock_panel gz_panel' data-type='panel' data-action='get_stock_panel' data-default='debug=1'>";
		$html.= "Stock panel...";
		$html.= "</div>";
		return $html;
	}
