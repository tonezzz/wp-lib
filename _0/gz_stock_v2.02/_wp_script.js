...
(function($){
	$('document').ready(function(){
		var $panel = $('.gz_stock_panel').gz_panel({ajax_url: gz_stock.ajax_url
			...
			//,callbacks: {
			//	tabs: function(e){console.log(e);}
			//}
			,callbacks:{
				init: function(obj){ //console.log(obj);
					if($(obj).attr('data-type')=='tabs' && !$(obj).data('tabs')) $(obj).tabs();
					return false;
				}
			}
		});
	...
	function prep_import_tabs(){
		$('.gz_stock_panel .db_import_tabs').on('refresh','.db_import_tabs',function(){console.log('[prep_import_tabs]',this);
		});
	}
})(jQuery);
