...
class gen_image extends gz_tpl{
	...
	function load_image(){//die(__FUNCTION__);
		...
		$img_data = file_get_contents($prm['file']); //ob_clean(); print_r($img_data); die();
		$img->readImageBlob($img_data);
		return $img;
	}
	...
	*/
	...
	function paste_image(){
		$prm = func_num_args()>0?func_get_arg(0):[]; //ob_clean(); echo "<pre>"; print_r($prm); die();
		...
		//ob_clean(); echo "<pre>"; var_dump(compact('src','dst')); die();
		if($prm_fit){
			$img_size = $dst->img->getImageGeometry(); $sx = $img_size['width']; $sy = $img_size['height'];
			$src->img->resizeImage($sx,$sy,Imagick::FILTER_POINT,1);
		}
	...
	function paste_image_0(){
		...
		$src = $this->parse_paste_param($prm['src']); //ob_clean(); echo "<pre>"; var_dump($src); die();
		$dst = $this->parse_paste_param($prm['dst']); //ob_clean(); echo "<pre>"; var_dump(compact('src','dst')); die();
		$this->image->compositeImage($src->img,Imagick::COMPOSITE_DEFAULT, $dst->x, $dst->y);
		unset($src->img); unset($src);
	}
	/*
	*	
	*/
	function draw_text_3(){//die(__FUNCTION__);
		$prm = wp_parse_args(func_num_args()>0?func_get_arg(0):[],[
			'text'	=> 'TEST'
		...
		$prm['font_file'] = $this->dir.'fonts/'.$prm['font']['name']; //ob_clean(); var_dump($prm['font_file']); die();
		//Set parameters
	...
	function do_c3(){//die(__FUNCTION__);
		$prm = func_num_args()>0?func_get_arg(0):[]; //ob_clean(); echo "<pre>"; var_dump($prm); die();
		$prm = $this->shortcode_atts(['file'=>false,'format'=>'jpeg','w'=>1000,'h'=>1000,'bg_color'=>false,'fit'=>false],$prm); //ob_clean(); echo "<pre>"; print_r($prm); die();
		extract($prm,EXTR_PREFIX_ALL,'prm'); //ob_clean(); echo "<pre>"; print_r($prm_file); die();
		$this->image_format = $prm_format; //die($this->image_format);
		...
		if($prm_file){
			...
			$this->paste_image(['src'=>['file'=>$prm_file],'dst'=>['img'=>$img],'fit'=>$prm_fit]);
		}
	...
	*/
	function render_image(){//die(__FUNCTION__);
		$seq_txt = $_REQUEST['seq']; //ob_clean(); echo '<pre>'; echo $seq; die();
		$seq_txt = stripslashes($seq_txt); //ob_clean(); echo '<pre>'; echo $seq; print_r(unserialize($seq)); die();
		//$seq_txt = serialize([['https://']]);
		$seq = unserialize($seq_txt); //ob_clean(); echo "<pre>"; print_r(compact('seq_txt','seq')); die();
		foreach($seq as $v){
			switch($v[0]){
...
//die(__FILE__);
		if(!isset($_REQUEST['e1'])){
			...
			$filename = empty($_REQUEST['filename'])?'product_image.jpg':$_REQUEST['filename'];
