...
class gz_tpl{
	...
	protected $script_loader_tag = []; //to add extra tag to <script>
	protected $style_loader_tag = []; //to add extra tag to <style>
	...
	private function init_script_loader_tag(){
		//if(!empty($this->script_loader_tag))
		//add_filter('script_loader_tag',[$this,'do_script_loader_tag'],999,2);
	}
	public function do_script_loader_tag($tag,$handle){ //ob_clean(); die('<pre>'.print_r(compact('tag','handle'),true));
		//if(is_array($tag) && sizeof($tag)>0){ob_clean(); die('<pre>'.print_r(compact('tag','handle'),true));
		//} //ob_clean(); die('<pre>'.print_r(compact('tag','handle'),true));
		//ob_clean(); echo sizeof($tag); die();
		if(isset($this->script_loader_tag[$handle])){
			$tag_st = $this->script_loader_tag[$handle];
			$tag = str_replace("></script>" ," {$tag_st}></script>",$tag);
		}
		return $tag;
	}
	...
	public function init_actions(){//ob_clean(); die('<pre>'.print_r($this->config['actions'],true));
		if(is_array($actions=$this->config['actions'])){
			foreach($actions as $item){ //ob_clean(); die('<pre>'.print_r($item['prm'],true));
	...
	*/
	...
	public function init_scripts(){//ob_clean(); die('<pre>'.print_r($this->config['enqueue'],true));
	...
	public function do_init_scripts($item){
		...
		switch($item['type']){
			...
			case 'destyle':
				...
				break;
			case 'script':
				//if(isset($item['tags'])) $this->script_loader_tag[$item['prm'][0]] = $this->arr_to_attr($item['tags']);
